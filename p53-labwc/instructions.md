...
Create the `~/.config/xdg-desktop-portal/labwc-portals.conf` file, with this content:
...
Then add the user to the `video` and `render` groups:
...
To be able to run `docker` without `sudo` add the user to the `docker` group:
...
Use a music/media player capable of bitperfect, for example `feishin`.
In the settings of the player:
- select `mpv` as backend
- select the right audio interface (suggested pipewire/alsa_*)
- select the `/usr/bin/mpv` executable
- enable audio exclusive mode
