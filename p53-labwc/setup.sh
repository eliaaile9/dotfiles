...
sudo pacman -S 7zip brightnessctl btop cifs-utils cliphist curl ddcutil discord evince evolution-data-center fastfetch feishin firefox foot fuzzel gimp grim imagemagick jdk21-openjdk jq less lua mpv neovim npm nwg-look obs-studio obsidian onefetch pacman-contrib parted pavucontrol playerctl python3 python rustup samba scrot slurp starship steam stow stylua swappy swaybg swaylock swaync tailscale tar telegram-desktop thunar ttf-terminus-nerd unrar unzip virtualbox virtualbox-host-modules-arch waybar wine wget wl-clipboard wlr-randr wlsunset xdg-desktop-portal zip
